...
function inject (bot) {
  ...
  function nextFrame (deltaSeconds) {
    ...
    // swimming
    const theCurrentBlockIAmIn = bot.blockAt(pos.offset(0, 1, 0))
    const inWater = theCurrentBlockIAmIn && theCurrentBlockIAmIn.type === 9;
    if (inWater && (controlState.jump || jumpQueued)) {


      vel.y = physics.waterTerminalVelocity
    }

    // jumping
    if ((controlState.jump || jumpQueued) && bot.entity.onGround && bot.entity.timeSinceOnGround > WAIT_TIME_BEFORE_NEW_JUMP && !inWater) {
    ...
    // gravity
    if (inWater){
      if(vel.y > physics.waterTerminalVelocity){
        acceleration.y += physics.waterDrag
      }else{
        acceleration.y -= physics.waterGravity
      }
    } else{
    ...
    if(inWater){
      if (acceleration.y < 0){
        vel.y = math.clamp(-physics.waterTerminalVelocity, vel.y, physics.waterTerminalVelocity)
      }
    }else{
